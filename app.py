...
class OptionsGreeksCalculator:
    ...
    def _parse_exchange_expiry(self, expiry_str: str) -> datetime:
        try:
            # Remove non-alphanumeric characters and uppercase the string
...
class IndexOptionsAnalyzer:
    ...
    def analyze_options(self, payload: Dict) -> Dict:
        try:
            ...
            # Reorganize the options data to extract 3 calls and 3 puts.
            reorganized_options = self.reorganize_options_data(current_market, current_price)
            # Process each option contract (fill in missing expiry if needed and calculate Greeks)
    ...
    def reorganize_options_data(self, current_market: Dict, spot: float) -> Dict[str, List[Dict]]:
        """
        Reorganizes the options data by flattening any "byExpiry" structure
        and then selecting the three contracts (calls and puts) whose strike prices
        are closest to the underlying spot.
        """
        ...
        # Sort each list by absolute difference between option strike and current price.
    ...
    def _flatten_options(self, options_data: Dict) -> Dict[str, List[Dict]]:
        """
        If options are provided under the "byExpiry" key, flatten them.
        Also, if an option contract is missing the 'expiry' field, assign it from the key.
        """
    ...
    def _process_contract(self, contract: Dict, spot: float, iv: float, futures: Dict) -> Dict:
        """
        Validates and processes a single option contract.
        If required fields are missing, it uses the trading symbol to fill them.
        Then it calculates the Greeks.
        """
    ...
    def _filter_atm_options(self, options: List[Dict], spot: float) -> List[Dict]:
        # Optionally, adjust this filter if needed.
        filtered = [p for p in options if 0.95 < (p.get('strikePrice', 0) / spot) < 1.05]
        return sorted(filtered, key=lambda x: abs(x.get('strikePrice', 0) - spot))[:3]
...
class ForecastingEngine:
    """
    Uses an ARIMA model and an LSTM (with MinMax scaling) to forecast the underlying price.
    """
...
class TradingStrategyEngine:
    """
    Generates trading strategies based on market analysis and forecast.
    It selects options, provides execution details, and risk management parameters.
    """
    def generate_strategies(self, analysis: Dict) -> Dict:
        try:
            historical_prices = analysis.get("historical_index_prices", [])
            forecast_info = {}
...
###############################################################################
# PART 5: Main for Heroku Deployment
###############################################################################
if __name__ == '__main__':
    # On Heroku, the PORT environment variable is set. Default to 5000 if not set.
